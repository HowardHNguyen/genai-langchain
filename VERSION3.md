...
Version 3 is a separate Streamlit app using `streamlit_url_app.py`. It has no file uploader.
Both entry points reuse the tested model, retrieval, citation, and rendering modules.

## Deploy separately

In Streamlit Community Cloud, create a **new app** with:

- Repository: `HowardHNguyen/genai-langchain`
- Branch: `main`
- Main file: `streamlit_url_app.py`
...
streamlit run streamlit_url_app.py
```
...
The test suite includes Version 2 regressions plus URL validation, DNS/private-network rejection, pinned TLS connections,
